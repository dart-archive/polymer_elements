...
main() async {
    ...
    group('basic', () {
        ...
        test('distributes nodes to topBar by default', () {
            ...
            HtmlElement insertionPoint = Polymer.dom(item).getDestinationInsertionPoints()[0];
            expect(Polymer.dom(insertionPoint).parentNode, equals(toolbar.$['topBar']));
        });
        ...
        test('distributes nodes with "middle" class to middleBar', () {
            ...
            HtmlElement insertionPoint = Polymer.dom(item).getDestinationInsertionPoints()[0];
            expect(Polymer.dom(insertionPoint).parentNode, equals(toolbar.$['middleBar']));
        });
        ...
        test('distributes nodes with "bottom" class to bottombar', () {
            ...
            HtmlElement insertionPoint = Polymer.dom(item).getDestinationInsertionPoints()[0];
            expect(Polymer.dom(insertionPoint).parentNode, equals(toolbar.$['bottomBar']));
        });
    });
    ...
    group('a11y', () {
        ...
        test('has role="toolbar"', () {
            ...
            expect(toolbar.getAttribute('role'), equals('toolbar'), reason: 'should have role="toolbar"');
        });

        test('children with .title becomes the label', () {
            PaperToolbar toolbar = fixture('title');
            expect(toolbar.getAttribute('aria-labelledby'), isNotNull, reason: 'should have aria-labelledby');
            expect(toolbar.getAttribute('aria-labelledby'), equals(toolbar.querySelector('.title').id), reason: 'aria-labelledby should have the id of the .title element');
        });

        test('existing ids on titles are preserved', () {
            PaperToolbar toolbar = fixture('title-with-id');
            expect(toolbar.getAttribute('aria-labelledby'), isNotNull, reason: 'should have aria-labelledby');
            expect(toolbar.querySelector('.title').id, equals('title'), reason: 'id should be preserved');
        });
        ...
        test('multiple children with .title becomes the label', () {
            ...
            expect(toolbar.getAttribute('aria-labelledby'), equals(ids.join(' ')), reason: 'aria-labelledby should have the id of all .title elements');
        });
    });
}