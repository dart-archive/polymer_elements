  ...
  group('<google-sheets>', () {
    ...
    test('published', () {
      ...
      expect(sheet.querySelector('google-signin-aware'), isNull,
          reason: 'google-signin-aware should not be created for a '
              'published sheet');
      ...
      sheet.on['google-sheet-data'].take(1).listen((e) {
        ...
        if (e.detail['type'] == 'tab') {
          ...
          expect(sheet.spreadsheets.length, 0,
              reason: '.spreadsheets length should be 0 since spreadsheet key '
                  'was given');
          expect(sheet.rows.length, greaterThan(0),
              reason: '.rows was not populated');
        }

        expect(sheet.$['cellrowsajax'].url, isNull,
            reason:
